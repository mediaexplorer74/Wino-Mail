...
namespace Wino.Core.Synchronizers
{
    ...
    {
        public override uint BatchModificationSize => 20;//1000;
        public override uint InitialMessageDownloadCountPerFolder => 30;//1200;
        ...
        {
            int totalDownloadCount = messageIds.Count() > 100 ? 100 : messageIds.Count();

            if (totalDownloadCount == 0) return;

            int downloadedItemCount = 0;

            _logger.Debug("Batch downloading {Count} messages for {Name}", 
                /*messageIds.Count()*/totalDownloadCount, Account.Name);

            var allDownloadRequests = messageIds.Select(CreateSingleMessageGet);

            // Respect the batch size limit for batch requests.
            IEnumerable<UsersResource.MessagesResource.GetRequest[]> batchedDownloadRequests = allDownloadRequests.Batch((int)MaximumAllowedBatchRequestSize);

            _logger.Debug("Total items to download: {TotalDownloadCount}. For {Name}, created {Count} batch download requests.",
                batchedDownloadRequests.Count(), Account.Name, totalDownloadCount);
            ...
            List<Task> batchProcessCallbacks = new List<Task>();
